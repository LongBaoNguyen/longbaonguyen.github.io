...
class Window:    
    ...
    def on_draw(self):
        ...
        # if right edge of ball passes right side or left side of screen negate change_x to change direction
        ...
        # 3) bottome side of ball is less than bottom side of paddle
        # change x direction if all above are true. 
        if left_side_ball < right_side_paddle and top_side_ball > top_side_paddle and bottom_side_ball < bottom_side_paddle:
            self.change_x *= -1  
        
        # if ball passes without hitting paddle, reset position of ball
        # to middle of screen.
        elif left_side_ball < right_side_paddle:
            self.x = WIDTH // 2
            self.y = HEIGHT // 2
